...
then
    echo "Usage: tests/test_theme.sh [XQ_BINARY] [OUTPUT_DIR] tests/99_foo.test"
    exit 1
fi
...
ENV=$(grep ENV $TEST_FILE | cut -b 4- | tr -d '\n')
ARGS=$(grep ARGS $TEST_FILE | cut -b 5- | tr -d '\n')
CMDS=$(grep CMDS $TEST_FILE | cut -b 5- | tr -d '\n')

eval "$ENV $PROG $ARGS $OUTPUT/${TEST_NAME}.input $CMDS render-tex | grep definecolor > $OUTPUT/${TEST_NAME}.output"
